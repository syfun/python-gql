...
class SchemaVisitor(Protocol):
    ...
    def visit_field_definition(
        self, field: GraphQLField, object_type: Union[GraphQLObjectType, GraphQLInterfaceType],
    ) -> GraphQLField:
        pass
...
def visit_schema(
    ...
    visitor_selector: Callable[[VisitableSchemaType, str], List["SchemaDirectiveVisitor"]],
) -> GraphQLSchema:
    """
    ...
    def call_method(
        ...
        for visitor in visitor_selector(type_, method_name):
            ...
            if method_name == "visit_schema" or isinstance(type_, GraphQLSchema):
                raise ValueError(f"Method {method_name} cannot replace schema with {new_type}")
    ...
    def visit(  # pylint: disable=too-many-return-statements
        ...
        if isinstance(type_, GraphQLInterfaceType):
            new_interface = cast(GraphQLInterfaceType, call_method("visit_interface", type_))
            if new_interface:
    ...
    def visit_fields(type_: Union[GraphQLObjectType, GraphQLInterfaceType]):
        def _update_fields(field, _):
            ...
            if new_field and new_field.args:
                update_each_key(
                    new_field.args,
                    lambda arg, _: call_method("visit_argument_definition", arg, new_field, type_),
                )
...
class SchemaDirectiveVisitor(SchemaVisitor):
    ...
    def get_declared_directives(
        cls, schema: GraphQLSchema, directive_visitors: Dict[str, Type["SchemaDirectiveVisitor"]],
    ):
        ...
        *,
        ...
        def _visitor_selector(
            ...
            for directive_node in directive_nodes:
                ...
                visitors.append(visitor_class(directive_name, args, type_, schema, context))
    ...
    def heal(type_: VisitableSchemaType):
        if isinstance(type_, GraphQLSchema):
            ...
            def _remove_dangling_references(_, type_name):
                if not type_name.startswith("__") and type_name not in actual_named_type_map:
                    return False
                return None
