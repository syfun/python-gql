...
from .scalars import ID, Upload, GraphQLUpload, JSONString, GraphQLJSONString
...
def get_graphql_type_for_annotation(annotation, field_name: str, force_optional: bool = False):
    ...
    if hasattr(annotation, "field"):
        ...
        if annotation_name == "List":
            list_of_type = get_graphql_type_for_annotation(annotation.__args__[0], field_name)

            field_type = GraphQLList(list_of_type)
        ...
        if annotation_origin == AsyncGenerator:
            ...
            if len(non_none_types) == 1:
                ...
                graphql_type = get_graphql_type_for_annotation(non_none_types[0], field_name, force_optional=True)
            else:
